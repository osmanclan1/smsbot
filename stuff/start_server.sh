...
# Set default AWS profile if not set (after loading .env)
if [ -z "$AWS_PROFILE" ]; then
    AWS_PROFILE=rise-admin4
fi
export AWS_PROFILE
...
# Prioritize AWS_PROFILE if set, otherwise use direct credentials
if [ -n "$AWS_PROFILE" ]; then
    ...
    if aws sts get-caller-identity --profile "$AWS_PROFILE" > /dev/null 2>&1; then
        ...
        if aws sso login --profile "$AWS_PROFILE" 2>&1; then
            ...
            if aws sts get-caller-identity --profile "$AWS_PROFILE" > /dev/null 2>&1; then
                IDENTITY=$(aws sts get-caller-identity --profile "$AWS_PROFILE" 2>/dev/null)
                ACCOUNT=$(echo "$IDENTITY" | grep -o '"Account": "[^"]*"' | cut -d'"' -f4)
                echo "✓ AWS credentials now valid - Account: $ACCOUNT"
            fi
        else
            echo "  ⚠️  SSO login failed or was cancelled"
    ...
    # Unset AWS_PROFILE to avoid conflicts with direct credentials
    unset AWS_PROFILE
    ...
    if aws sts get-caller-identity > /dev/null 2>&1; then
        ...
        echo "  Or remove them and use AWS_PROFILE instead"
    fi
else
    echo "⚠️  Warning: No AWS credentials found"
    echo "  Please set AWS_PROFILE or AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY in .env"
fi
