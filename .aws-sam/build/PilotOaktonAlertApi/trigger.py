...
def get_trigger_message(trigger_type: str, days: Optional[int] = None) -> str:
    if trigger_type == "payment_deadline_reminder" and days is not None:
        return messages.TRIGGER_MESSAGES["payment_deadline_reminder"].format(days=days)
    if trigger_type in messages.TRIGGER_MESSAGES:
        return messages.TRIGGER_MESSAGES[trigger_type]
