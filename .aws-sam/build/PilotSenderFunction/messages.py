...
def format_trigger_message(trigger_type: str, days: int | None = None) -> str:
    """Build outbound SMS body for a trigger type (same rules as /api/trigger)."""
    if trigger_type == "payment_deadline_reminder":
        d = days if days is not None else 3
        return TRIGGER_MESSAGES["payment_deadline_reminder"].format(days=d)
    if trigger_type in TRIGGER_MESSAGES:
        return TRIGGER_MESSAGES[trigger_type]
